...
temp_run2 = list.files("Data/Run with forecasting 1 period/Direct Forecast/Run2/", pattern="*.csv")
...
for (file in temp_run2){
  nn_temp = read.csv(paste0('Data/Run with forecasting 1 period/Direct Forecast/Run2/',file))
  if(grepl('absolute', file, fixed=TRUE)){
    if(grepl('combination', file, fixed=TRUE)|grepl('Combination', file, fixed=TRUE)){
      nn_comb <- rbind(nn_comb, nn_temp)
    }
...
results_df$ROI[is.nan(results_df$ROI)]<-0
